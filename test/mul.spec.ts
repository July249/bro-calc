...
describe('mul function', () => {
  ...
  it('even the argument of mul function is over the 2^53 - 1, it should be calculated correctly', () => {
    ...
    expect(test3.mul(test4)).toMatchObject({
      d: [152415, 7875323, 8836750, 4953515, 4031397, 6765279, 6829976, 5279684],
      e: 0,
      s: 1,
    })
    expect(test3.mul(test4).toString()).toBe('1524157875323883675049535154031397676527968299765279684')
  ...
  it('multiply many digits', () => {
    const x1 = new Numbra('99999999999999999999999999999999999999999999999999999')
    const x2 = new Numbra('0.0000000000000000000000000000000000000000000000000001')
    expect(x1.mul(x2)).toMatchObject({
      d: [9999, 9999999, 9999999, 9999999, 9999999, 9999999, 9999999, 9999999],
      e: -52,
      s: 1,
    })
    expect(x1.mul(x2).toString()).toBe('9.9999999999999999999999999999999999999999999999999999')
  })
})
